...
resource "azurerm_lb_rule" "cf-balancer-rule-https" {
  ...
  resource_group_name            = "${azurerm_resource_group.rg.name}"
  depends_on                     = ["azurerm_lb_probe.web-https-probe", "azurerm_lb_backend_address_pool.cf-balancer-backend-pool"]
  loadbalancer_id                = "${azurerm_lb.cf-balancer.id}"
  protocol                       = "Tcp"
  frontend_port                  = 443
  backend_port                   = 443
...
resource "azurerm_lb_rule" "cf-balancer-rule-http" {
  ...
  resource_group_name            = "${azurerm_resource_group.rg.name}"
  depends_on                     = ["azurerm_lb_probe.web-http-probe", "azurerm_lb_backend_address_pool.cf-balancer-backend-pool"]
  loadbalancer_id                = "${azurerm_lb.cf-balancer.id}"
  protocol                       = "Tcp"
  frontend_port                  = 80
  backend_port                   = 80
